...
def start_toil(dataset_name, options, use_data=False):
    if use_data:
        data = Job.wrapJobFn(download_data.start_toil)
        mmdb2pdb = data.addFollowOnJobFn(convert_mmdb_to_pdb.start_toil)
    else:
        mmdb2pdb = Job.wrapJobFn(convert_mmdb_to_pdb.start_toil)

    interactome = mmdb2pdb.addChildJobFn(get_structural_interactome.start_toil, dataset_name)
    bsa = interactome.addFollowOnJobFn(calculate_bsa.start_toil, dataset_name)

    #prep_protein = mmdb2pdb.addChildJobFn(prepare_protein.start_toil, dataset_name)
    features = mmdb2pdb.addFollowOnJobFn(calculate_features.start_toil, dataset_name, name="features") #prep_protein

    filter = mmdb2pdb.addFollowOnJobFn(filter_dataset.start_toil, dataset_name, name="filter")
