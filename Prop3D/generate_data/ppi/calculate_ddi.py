...
def prep(*pdbs, **kwds):
    import subprocess
    from Prop3D.generate_data.util import PDB_TOOLS
...
def download_pdb(job, sfam_id, pdb, chain, sdi, domNo, extracted=False, work_dir=None):
    ...
    in_store = IOStore.get("aws:us-east-1:Prop3D-full-structures")
    ...
    if mol_pdb == int_pdb and mol_chain == int_chain:
        ...
        if nbr is not None:
            ...
            if not os.path.isfile(obs_file):
                obs_store = IOStore.get("aws:us-east-1:Prop3D-interfaces-1")
                try:
...
def process_interface(job, row, int_type, pdbFileStoreID, download=True, work_dir=None):
    from Prop3D.parsers.superpose import align
...
def advanced_dock(job, mol_file, mol_resi, int_file, int_resi, original_complex, int_id=None, rerun=False, work_dir=None, cores=1):
    from Prop3D.parsers.haddock import dock
...
def simple_dock(job, orig_file, mol_resi, int_resi, original_complex, rerun=False, work_dir=None):
    from Prop3D.parsers.CNS import Minimize
...
def get_original_complexes(job, mol_sfam, int_sfam, group, int_type, pdbFileStoreID, work_dir=None):
    ...
    ddi_store = IOStore.get("aws:us-east-1:Prop3D-ddi")
...
def cluster(job, mol_sfam, int_sfam, pdb_complexes, work_dir=None):
    from Prop3D.parsers.MaxCluster import run_maxcluster, get_centroid
    from Prop3D.generate_data.util import PDB_TOOLS, SubprocessChain
...
#     from Prop3D.parsers.CNS import Minimize, calculate_energy
#     t0 = time.time()
#     complex_file_orig = next(prep((pdb1, "M"), (pdb2, "I"), merge=True))
...
#     store = IOStore.get("aws:us-east-1:Prop3D-ddi")
#
#     assert 0, runs
#
...
#     work_dir = job.fileStore.getLocalTempDir()
#     #prefix = job.fileStore.jobStore.config.jobStore.rsplit(":", 1)[0]
#     in_store = IOStore.get("aws:us-east-1:Prop3D-ddi")
#
...
def process_sfam_sfam(job, group, pdbFileStoreID, int_type, rerun=False, cores=1):
    ...
    work_dir = job.fileStore.getLocalTempDir()
    ddi_store = IOStore.get("aws:us-east-1:Prop3D-ddi")
    interface_store = IOStore.get("aws:us-east-1:Prop3D-interfaces-1")
...
def best_sfams(job, all_counts, max_sfams=300):
    import json
    work_dir = job.fileStore.getLocalTempDir()
    out_store = IOStore.get("aws:us-east-1:Prop3D-ddi")
...
def start_toil(job, observed=True, run_haddock=True, memory="1G"):
    work_dir = job.fileStore.getLocalTempDir()
    store = IOStore.get("aws:us-east-1:Prop3D-ddi")
    pdb_store = IOStore.get("aws:us-east-1:Prop3D-ibis")
    ddi_store = IOStore.get("aws:us-east-1:Prop3D-ddi")
    ...
    if not store.exists("sorted_sfams.json"):
        interface_store = IOStore.get("aws:us-east-1:Prop3D-interfaces")
