...
class Minimize(CNS):
    ...
    def __call__(self, pdb_file, out_file_prefix=None):
        return self.run(pdb_file=pdb_file, out_file_prefix=out_file_prefix)

    def run(self, pdb_file, out_file_prefix=None):
...
#     cns = CNS(temp_minimization_file, output_file_prefix, work_dir=work_dir, docker=docker,
#         job=job, INSERT_PDB_HERE=pdb_file, INSERT_OUTPUT_FILE=minimized_file)
#
